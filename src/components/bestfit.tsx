...
function Bestfit() {
    ...
    useEffect(() => {
        ...
        const generateDataWithoutVarianz = (numPoints) => {
            setScatterData([]);
            ...
            return { x: xValuesTensor2D, y: yValuesWithoutNoise };
        };

        function createModel() {
            const model = tf.sequential();

            // Add layers according to the provided parameters
            model.add(tf.layers.dense({ inputShape: [1], units: 1, activation: 'linear', useBias: true }));
            model.add(tf.layers.batchNormalization());

            model.add(tf.layers.dense({ units: 15, activation: 'relu', useBias: true }));
            model.add(tf.layers.batchNormalization());
            model.add(tf.layers.dense({ units: 10, activation: 'softmax', useBias: true }));
            model.add(tf.layers.batchNormalization());
